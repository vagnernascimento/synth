...
class NodeDecorator
  ...
  def node_position
    @node_position ||= context.resources.to_a.index(RDFS::Resource.new(self.uri)) || 0
  end
    
  def next_node
    @next_node ||= node_position == (context.resources.to_a.size - 1) ? nil : context.node(context.resources.to_a[node_position + 1])
  end

  def next_node_anchor    
    @next_node_anchor ||= node_position == (context.resources.to_a.size - 1) ? nil : next_node.anchor
  end
  ...
  def previous_node
    @previous_node ||= node_position == 0 ? nil : context.node(context.resources.to_a[node_position - 1])
  end
